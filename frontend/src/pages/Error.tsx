import Page from "@/components/wrapper/Page";
import { useNavigate } from "react-router-dom";

const Error = () => {
  const navigate = useNavigate();
  return (
    <Page className="flex justify-center items-center">
      <div className="flex flex-col bg-grayblack justify-around items-center gap-x-4">
        <p className="text-[12rem] font-bold tracking-wider text-white mx-4">404</p>

        <div className="text-white flex flex-col items-center gap-y-4 mx-4">
          <h2 className="my-2 font-bold text-2xl text-center">
            Looks like you've found the doorway to the great nothing
          </h2>
          <p className="my-2 text-center">Sorry about that! Please visit our hompage to get where you need to go.</p>
          <button
            className="w-auto mt-16! border px-10! py-6! text-center
          hover:text-black hover:bg-white transition-all ease-in-out"
            onClick={() => navigate("/")}
          >
            Take me there!
          </button>
        </div>
      </div>
    </Page>
  );
};

export default Error;
