...
export default function NotesPage() {
  ...
  const folders = useAppSelector((state) => state.noteFolderList.items);
  // undefined on the index route (`/notes`), which has no detail loader
  const data = useLoaderData<NoteDetailLoaderData | undefined>();
  return (
    <NotesScreen notes={notes} folders={folders} loadedNote={data?.note ?? null} backlinks={data?.backlinks ?? []} />
  );
}
